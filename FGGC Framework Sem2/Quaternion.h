...
public:
	...
	*/
	...
	void SetQuaternion(Vector3 degs)
	{
		Vector3 rads = degs * (XM_PI / 180);

		float cy, sy, cp, sp, cr, sr;

		cy = (float)cos(rads.z * 0.5);
		sy = (float)sin(rads.z * 0.5);
		cp = (float)cos(rads.y * 0.5);
		sp = (float)sin(rads.y * 0.5);
		cr = (float)cos(rads.x * 0.5);
		sr = (float)sin(rads.x * 0.5);
